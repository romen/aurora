...
pub(super) const NAMES: &CStr =
    c"SLH-DSA-SHAKE-128f:2.16.840.1.101.3.4.3.27:id-slh-dsa-shake-128f:slhdsa_shake_128f";

/// NAME should be a substring of NAMES
pub(crate) const NAME: &CStr = c"SLH-DSA-SHAKE-128f";
...
pub(crate) const SIGALG_OID: Option<&CStr> = Some(c"2.16.840.1.101.3.4.3.27");
...
// Ensure proper null-terminated C string
pub(super) const DESCRIPTION: &CStr = c"SLH-DSA-SHAKE-128f from rustcrypto";
...
pub(crate) mod capabilities {
    pub(crate) mod tls_sigalg {
        ...
        /// A [_unit-like struct_][rustbook:unit-like-structs] implementing [`TLSSigAlg`] for `id-slh-dsa-shake-128f`.
        ///
        ...
        impl TLSSigAlg for TLSSigAlgCap {
            ...
            /// > For SLH-DSA we currently refer to ids reserved by <https://datatracker.ietf.org/doc/draft-reddy-tls-slhdsa/01/>
            /// > as IANA does not list SLH-DSA in the registry yet.
            const SIGALG_IANA_NAME: &CStr = c"slhdsa_shake_128f";
            ...
            const SIGALG_NAME: &CStr = c"SLH-DSA-SHAKE-128f";
