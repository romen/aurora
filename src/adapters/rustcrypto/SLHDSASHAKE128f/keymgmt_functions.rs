...
use signature_traits::{Keypair, Signer, VerificationError, Verifier};
...
use slh_dsa as backend_module;
use typenum::Unsigned; // needed to fetch the expected sizes as USIZE
type InnerParamSet = backend_module::Shake128f;
type InnerPubKey = backend_module::VerifyingKey<InnerParamSet>;
type InnerPrivKey = backend_module::SigningKey<InnerParamSet>;
type InnerSig = backend_module::Signature<InnerParamSet>;
...
#[derive(PartialEq)]
pub struct PrivateKey(InnerPrivKey);

impl PublicKey {
    pub fn decode(bytes: &[u8]) -> Result<Self, KMGMTError> {
        let k = InnerPubKey::try_from(bytes)
            .map_err(|e| anyhow!("InnerPubKey::try_from(bytes) returned {:?}", e))?;
        Ok(Self(k))
    }
    ...
    pub const fn byte_len() -> usize {
        <InnerParamSet as slh_dsa::VerifyingKeyLen>::VkLen::USIZE
    }
...
impl Verifier<Signature> for PublicKey {
    ...
    fn verify(&self, msg: &[u8], sig: &Signature) -> Result<(), signature_traits::Error> {
        ...
        pk.try_verify_with_context(msg, &ctx, signature)
            .map_err(|e| {
                error!(target: log_target!(), "{e:?}");
...
impl PrivateKey {
    pub fn encode(&self) -> Vec<u8> {
        ...
        let ret = sk.to_vec();
        debug_assert_eq!(ret.len(), SECRETKEY_LEN);
        ret
    }
    ...
    pub const fn byte_len() -> usize {
        <InnerParamSet as backend_module::SigningKeyLen>::SkLen::USIZE
    }

    pub const fn signature_bytes() -> usize {
        <InnerParamSet as backend_module::SignatureLen>::SigLen::USIZE
    }
    ...
    pub fn from_DER(sk_der_bytes: &[u8]) -> OurResult<(Self, Option<PublicKey>)> {
        ...
        // According to the ASN.1 module definition, rasn would expect a DER encoded OCTET STRING as the input here.
        // But, at least looking at OpenSSL 3.5 encodings, we actually get the raw bytes.
        ...
        //     }
        // };
        // let raw_privkey_bytes: &[u8] = &decodedprivkey;
        let raw_privkey_bytes: &[u8] = sk_der_bytes;

        debug!(target: log_target!(), "Parsed private key material out of ASN.1 for decoding!");

        let privkey = keymgmt_functions::PrivateKey::decode(raw_privkey_bytes)?;

        // We are always able to retrieve the pubkey from an encoded SLH-DSA private key.
        //
        // Nonetheless this function is expected to return the pubkey wrapped in
        // an Option for compatibility with other signature algorithms which
        // cannot guarantee this from the encoding of their private key (I am
        // looking at you ML-DSA)

        let pubkey: PublicKey = {
            let innerpubkey = privkey.0.verifying_key();
            PublicKey(innerpubkey)
        };
    ...
    pub fn to_DER(&self) -> OurResult<Vec<u8>> {
        ...
        // according to the ASN.1 module, rasn gives us a DER encoded OCTET
        // STRING, but we need to pass along only the inner VALUE, stripping TAG
        // and LEN.
        let asn_sk_bytes = match get_DER_inner_value(&asn_sk_bytes) {
            Ok(v) => v.to_vec(),
...
pub(super) mod asn_definitions {
    pub use crate::asn_definitions::x509_slh_dsa_module_2024 as defns;

    pub use defns::SLHDSAPrivateKey as PrivateKey;
    pub use defns::SLHDSAPublicKey as PublicKey;
}

/// This utility function is a helper to strip TAG and LEN
/// from a DER encoded TLV.
fn get_DER_inner_value(der_encoded_TLV: &[u8]) -> ::der::Result<&[u8]> {
    // rasn does not expose an API to do this, so we resort to the `der`
    // crate for this operation.
    use ::der::{AnyRef, Decode, DecodeValue};

    let any = AnyRef::from_der(der_encoded_TLV)?;
