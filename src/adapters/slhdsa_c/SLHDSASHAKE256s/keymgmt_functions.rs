...
type InnerParamSet = backend_module::SLH_DSA_SHAKE_256s;
...
impl PublicKey {
    ...
    pub const fn byte_len() -> usize {
        <InnerParamSet as backend_module::VerifyingKeyLen>::LEN::USIZE
    }
...
impl PrivateKey {
    ...
    pub const fn byte_len() -> usize {
        <InnerParamSet as backend_module::SigningKeyLen>::LEN::USIZE
    }
...
impl<'a> KeyPair<'a> {
    ...
    fn generate(provctx: &'a ProviderInstance) -> Result<Self, KMGMTError> {
        ...
        let (sk, pk) =
            InnerParamSet::keygen().map_err(|_err| anyhow!("Keygen failed on backend"))?;
...
mod tests {
    ...
    fn const_sanity_assertions() {
        ...
        assert_eq!(SECRETKEY_LEN, 128);
        assert_eq!(SIGNATURE_LEN, 29792);
