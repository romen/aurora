...
pub(super) const NAMES: &CStr =
    c"SLH-DSA-SHAKE-256s:2.16.840.1.101.3.4.3.30:id-slh-dsa-shake-256s:slhdsa_shake_256s";

/// NAME should be a substring of NAMES
pub(crate) const NAME: &CStr = c"SLH-DSA-SHAKE-256s";

/// LONG_NAME should be a substring of NAMES
pub(crate) const LONG_NAME: &CStr = c"id-slh-dsa-shake-256s";

/// OID should be a substring of NAMES
pub(crate) const OID: asn1::ObjectIdentifier = asn1::oid!(2, 16, 840, 1, 101, 3, 4, 3, 30);
pub(crate) const OID_PKCS8: pkcs8::ObjectIdentifier =
    pkcs8::ObjectIdentifier::new_unwrap("2.16.840.1.101.3.4.3.30");
pub(crate) const SIGALG_OID: Option<&CStr> = Some(c"2.16.840.1.101.3.4.3.30");
...
// Ensure proper null-terminated C string
pub(super) const DESCRIPTION: &CStr = c"SLH-DSA-SHAKE-256s from slhdsa_c_rs";
...
pub(crate) mod capabilities {
    pub(crate) mod tls_sigalg {
        ...
        /// A [_unit-like struct_][rustbook:unit-like-structs] implementing [`TLSSigAlg`] for `id-slh-dsa-shake-256s`.
        ///
        ...
        impl TLSSigAlg for TLSSigAlgCap {
            ...
            /// > For SLH-DSA we currently refer to ids reserved by <https://datatracker.ietf.org/doc/draft-reddy-tls-slhdsa/01/>
            /// > as IANA does not list SLH-DSA in the registry yet.
            const SIGALG_IANA_NAME: &CStr = c"slhdsa_shake_256s";
            ...
            /// > For SLH-DSA we currently refer to ids reserved by <https://datatracker.ietf.org/doc/draft-reddy-tls-slhdsa/01/>
            /// > as IANA does not list SLH-DSA in the registry yet.
            const SIGALG_CODEPOINT: u32 = 0x091B; // 2331 in decimal notation
            ...
            const SIGALG_NAME: &CStr = c"SLH-DSA-SHAKE-256s";

            /// The OID of the [`Self::SIGALG_SIG_NAME`] algorithm in canonical numeric text form. \[optional\]
            ///
            /// # NOTE
            ///
            /// > The OIDs for SLH-DSA come from the [NIST Computer Security Objects Register](https://csrc.nist.gov/projects/computer-security-objects-register/algorithm-registration).
